...
)
{
    ...
    for (int k = 1; k < n; k *= 2) {
            ...
            [&](int i, int j)
        {
            ...
            const int
                equivalence_class_i_second =
                    (i + k < n)
                    ? equivalence_class[i + k]
                    : -1;

            const int
                equivalence_class_j_second =
                    (j + k < n)
                    ? equivalence_class[j + k]
                    : -1;
...
)
{
    ...
    for (
        ...
        for (int position :
             second_key_order)
        {
            const int current_class =
                equivalence_class[position];

            new_suffix_array[
                class_start[current_class]++
            ] = position;
        }
        ...
        for (int i = 1; i < n; ++i) {
            ...
            const int previous =
                new_suffix_array[i - 1];

            const int current =
                new_suffix_array[i];

            const int previous_first =
                equivalence_class[previous];

            const int current_first =
                equivalence_class[current];

            const int previous_second =
                (previous + k < n)
                ? equivalence_class[
                    previous + k
                ]
...
{
    return build_suffix_array_optimized(
        text
    );
}